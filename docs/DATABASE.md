...
| `00008_username_check_and_grants.sql` | `profiles.username` format `CHECK (~ '^[a-zA-Z0-9_]{3,30}$')` (added `NOT VALID` so a manual apply can't fail on legacy rows; enforced on all future writes); re-asserts `accept_invite(uuid)` execute grant (defense-in-depth — 00005/00006 redefined it via `CREATE OR REPLACE`). **Pending manual apply.** |

> **Live-DB state (2026-07-15, applied + verified via MCP):** `00001`–`00007` are all applied. On 2026-07-15 the live DB had only `00001`–`00004`; `00005`, `00006`, `00007` were applied via the Supabase MCP connection and verified (policies/RPCs/trigger/indexes present). **`00008` is not yet applied** — apply it by hand after `00007`.
